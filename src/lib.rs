...
where
    C: Service<Uri> + Clone + Unpin + Send + 'static,
    ...
    C::Future: Unpin + Send,
{
    ...
    type Future = ServiceConnecting<C::Response>;
    ...
    fn call(&mut self, uri: Uri) -> Self::Future {
        ...
        let f = async move {
            let uri = match (&resolver, uri.host(), uri.port()) {
                (Some(resolver), Some(host), None) => {
                    let res = resolver.srv_lookup(host).await;
                    ...
                    match response.as_ref().and_then(|response| response.iter().next()) {
                        Some(srv) => {
            ...
            inner.call(uri).await.map_err(ServiceError::inner)
        };
        ServiceConnecting(Box::pin(f))
    }
}
...
///
/// [`ServiceConnector`]: struct.ServiceConnector.html
pub struct ServiceConnecting<S>(BoxFuture<'static, Result<S, ServiceError>>);

impl<S> fmt::Debug for ServiceConnecting<S> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        f.debug_struct("ServiceConnecting").finish()
    }
}

impl<S> Future for ServiceConnecting<S> {
    type Output = Result<S, ServiceError>;
